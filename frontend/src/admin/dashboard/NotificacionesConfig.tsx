...
export default function NotificacionesConfig() {
  ...
  function handleSaveTime() {
    const t = timeInput || config?.notificationTime
    if (!t) return
    updateConfig.mutate({ notificationTime: t })
    setTimeInput('')
  }
      ...
      <div className="p-4 flex flex-col gap-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-muted mt-0.5">Recibi un aviso la noche anterior con los pedidos del dia siguiente</p>
          </div>
          <button
            type="button"
            onClick={handleToggle}
            disabled={!config || updateConfig.isPending}
            className={`relative w-11 h-6 rounded-full transition-colors duration-200 shrink-0 disabled:opacity-40 ${config?.enabled ? 'bg-terra' : 'bg-sand-deep'}`}
          >
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1">
            ...
            <input
              type="time"
              value={timeInput || config?.notificationTime || '21:00'}
          ...
          <button
            ...
            disabled={updateConfig.isPending || (!timeInput && !config?.notificationTime)}
            className="mt-5 flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold bg-terra text-white disabled:opacity-40 disabled:cursor-not-allowed hover:bg-terra-dark transition-colors"
          >
        ...
        <div className="border-t border-sand pt-4">
          ...
          {!isSupported ? (
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold border border-sand-deep text-brown disabled:opacity-40 hover:border-terra hover:text-terra transition-colors"
            >
