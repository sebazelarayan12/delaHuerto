...
export default function PedidosPage() {
  ...
  const totalHistorialPages = Math.ceil(historialFiltrado.length / PAGE_SIZE)
  ...
  const FILTRO_PILLS: { key: FiltroActivos; label: string }[] = [
  ...
  return (
    <AdminLayout>
      ...
      <div className="px-4 lg:px-8 py-6 flex flex-col gap-5">
        ...
        {/* Activos */}
        <div>
          <div className="flex items-center justify-between mb-[14px] flex-wrap gap-y-2">
            ...
            <div className="flex items-center gap-1.5">
              {FILTRO_PILLS.map(({ key, label }) => (
                <button
                  key={key}
                  type="button"
                  onClick={() => setFiltroActivos(key)}
                  className={`px-3 py-1 rounded-full text-[12px] font-bold border transition-colors cursor-pointer font-sans ${
                    filtroActivos === key
                      ? 'bg-terra text-white border-terra'
                      : 'bg-transparent text-brown border-sand-deep hover:border-terra hover:text-terra'
                  }`}
                >
                  {label}
                </button>
            ...
            <div className="mt-3 flex flex-col gap-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => { setBusquedaHistorial(e.target.value); setHistorialPage(1) }}
