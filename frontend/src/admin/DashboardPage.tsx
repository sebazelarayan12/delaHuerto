...
export default function DashboardPage() {
  ...
  return (
    <AdminLayout>
      <div>
        <div style={{ padding: '32px 32px 0', borderBottom: '1px solid #E2CFB5' }}>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: '#2C1208' }}>Buen día, Carmen 👋</h1>
            ...
            {KPIS.map((k) => (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <span className="icon icon-fill" style={{ color: k.color, fontSize: 22 }}>{k.icon}</span>
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#9A7A66', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{k.label}</span>
                </div>
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 800, color: '#2C1208' }}>{k.value}</div>
              </div>
            ))}
          </div>
              ...
              {unavailableProds.length === 0 ? (
                ...
                unavailableProds.slice(0, 5).map((p) => (
                  ...
                  >
                    ...
                    <span style={{ background: '#fef2f2', color: '#dc2626', borderRadius: 99, padding: '2px 8px', fontSize: 11, fontWeight: 700 }}>
                      Sin stock
                    </span>
                  </div>
                ))
