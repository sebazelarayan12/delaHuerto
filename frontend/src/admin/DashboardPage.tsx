...
export default function DashboardPage() {
  ...
  const { data: categorias } = useQuery({
    ...
    queryFn: async () => (await api.get<Categoria[]>('/api/admin/categorias')).data,
  })
        ...
        <div className="px-4 lg:px-8 pt-6 lg:pt-8 flex justify-between items-start border-b border-sand-deep pb-5">
          <div>
            ...
            <p className="text-sm text-muted mt-1">
              Esto es lo que esta pasando hoy con tu menu.
            </p>
          </div>
          <a
            href="/"
            ...
            className="inline-flex items-center gap-2 bg-terra text-white px-4 py-2 rounded-lg no-underline text-sm font-semibold shrink-0"
          >
              ...
              >
                ...
                {k.sparkline ? (
                  <div className="flex items-flex-end justify-between">
                    <div className={`font-display text-[22px] font-extrabold ${k.textClass}`}>{k.value}</div>
                    <div className="flex items-end">
                      <Sparkline data={k.sparkline} color={k.color} width={100} height={36} />
                    </div>
                  </div>
                ) : (
